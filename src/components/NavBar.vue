...
<div>
      ...
      <nav class="navbar navbar-light bg-white">
        <div class="d-flex align-items-center">
          ...
          <div class="delivery-pickup-main d-flex align-items-center" v-if="!is_scrren_hide_mobile">
            <div class="delivery-pickup-lnk" v-if="this.$route.name == 'Home' && $auth.setting && $auth.setting.is_pickup_enable && !is_scrren_hide_mobile">
            ...
            <a href="javascript:void(0)" @click="deliveryModalAddress" class="delivery-link map-location set-address-link" v-if="this.$route.name == 'Home' && !is_scrren_hide_mobile">
              ...
              <span v-if="address == ''" > Set Location </span>
          ...
          <div class="logo-main">
            <router-link to="/home" class="desktop-logo"> <img class="nav-logo" :src="$auth.getAppLogo()" /></router-link>
            <router-link to="/home" class="mobi-logo"> <img class="nav-logo" :src="$auth.getAppLogo()" /></router-link>
          </div>
        </div>
        <nav-right></nav-right>
      </nav>
  ...
  <side-bar></side-bar>
  <!-------New MOdal------->
  <div class="modal fade" id="deliveryModal" tabindex="-1" aria-labelledby="deliveryModallabel" aria-hidden="true" v-if="!is_scrren_hide_mobile">
              ...
              <div class="orders-lst">
                <div class="inner-data">
                  <div class="saved-address-main">
                    <div class="d-flex justify-content-between" v-for="(addres, index) in this.getAddresses" :key="index" @click="selectDefaultAddress(addres)">
                      <div class="saved-address w-100 nav-saved-address" @click=" OnAddressList($event,addres.adress_clarification,addres.flat_no,addres.house_name,addres.Shiping_address,addres.latitude,addres.longitude,addres.city,addres.state)">
                        <p class="text-capitalize mb-1">
                          {{ addres.adress_clarification }}
                        </p>
                        <span v-if="addres.flat_no == null || addres.flat_no == ''">{{addres.house_name +" " +addres.Shiping_address}}</span>
